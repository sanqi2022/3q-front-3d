...
function SidebarScript(editor) {
  ...
  var addPoint = new UIButton('添加点')
  addPoint.onClick(function () {
    if(editor.selected && editor.selected.name === 'Line') {
      let li = editor.selected
      let points = li.userData.points
      let p = new THREE.Vector3(0, 2.0, 2.0)
      points.push(p)
      ////
      let xian = li.children[0]
      const position = xian.geometry.attributes.position
      position.setXYZ(points.length - 1, p.x, p.y, p.z)
