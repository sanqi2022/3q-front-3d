...
function Viewport(editor) {
  ...
  transformControls.addEventListener('change', function () {
    ...
    if (object !== undefined) {
      ...
      // 线移动
      let line = object.userData.line
      let index = object.userData.index
      if (line && index >= 0) {
        const position = line.geometry.attributes.position
        position.setXYZ(index, object.position.x, object.position.y, object.position.z)
        position.needsUpdate = true
        // signals.refreshSidebarObject3D.dispatch(line);
      }
    }
